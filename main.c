...
static int run_recv(int argc, char **argv)
{
    ...
    int follow = 0, all = 0, stale = 0, t, i;
    for (i = 2; i < argc; i++) {
        if (strcmp(argv[i], "--follow") == 0 || strcmp(argv[i], "-f") == 0) follow = 1;
        else if (strcmp(argv[i], "--all") == 0 || strcmp(argv[i], "-a") == 0) all = 1;
        else if (strcmp(argv[i], "--exit-alone") == 0 || strcmp(argv[i], "-e") == 0) {
            stale = 600;                       /* T follows -e directly, or defaults */
    ...
    if (me == NULL || !ok_name(me)) return die("usage: iac recv <room> <me> [seconds] [--follow] [--all] [--exit-alone [T]]");
    t = (secs != NULL) ? atoi(secs) : 60;
    if (t < 0) t = 0;
    return follow ? cmd_follow(room, me, t) : cmd_recv(room, me, t, all, stale);
}
...
static void usage(FILE *out)
{
    fputs(
        ...
        "                                   -e/--exit-alone [T]: exit 3 when no other member is active within T secs, default 600)\n"
